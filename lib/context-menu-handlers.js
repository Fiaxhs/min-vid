...
const getTwitchUrl = require('./get-twitch-url');
...
function init() {
  ...
  const onMessageTwitchHandler = (url) => {
    sendMetricsData({changed: 'activate', domain: 'twitch.tv'});
    launchVideo({url: url,
                 domain: 'twitch.tv',
                 getUrlFn: getTwitchUrl});
  }
  ...
  getItem({
    label: contextMenuLabel,
    context: cm.SelectorContext(getSelectors('twitch')),
    contentScript: contextMenuContentScript,
    onMessage: onMessageTwitchHandler
  });
}
